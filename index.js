...
async function update() {
    ...
    (await axios.get(`${dynv6ApiEndpoint}/zones/${zone.data.id}/records`, { headers: { 'Authorization': `Bearer ${process.argv[httpTokenIndex]}` } })).data
        .filter((record) => record.name !== '').forEach((record) => axios.patch(`${dynv6ApiEndpoint}/zones/${zone.data.id}/records/${record.id}`, { data: ipv6Address }, { headers: { 'Authorization': `Bearer ${process.argv[httpTokenIndex]}` } }).then(response => console.log(`${new Date().toLocaleTimeString()}: ${response.data.name}.${process.argv[hostnameIndex]} address updated`)));
}
consume(update());
